// SkyVanta AI — Standalone C++ Kalman Demo & HUD Drawing Subsystem
// Creator: SkyVanta-AI / Devendraprasad
...
inline void glow_circle(cv::Mat& img, cv::Point2d center, double radius,
    ...
    cv::addWeighted(overlay, 0.5 * intensity, img, 1.0 - 0.5 * intensity, 0, img);
...
inline void put_text(cv::Mat& img, const std::string& text, cv::Point org,
    ...
    if (glow) {
        cv::putText(img, text, org, cv::FONT_HERSHEY_SIMPLEX, scale, cv::Scalar(0, 0, 0), thickness + 2, cv::LINE_AA);
    }
...
inline void frame_corners(cv::Mat& img, int margin = 24, int len = 26,
    ...
    for (auto& c : std::vector<std::array<int, 4>>{{margin, margin, 1, 1}, {w - margin, margin, -1, 1},
        ...
        cv::line(img, {c[0], c[1]}, {c[0] + c[3] * len, c[1]}, color, thickness, cv::LINE_AA);
    }
}
...
public:
    ...
    void push(cv::Point2d pt) {
        pts_.push_back(pt);
        while (pts_.size() > max_len_) pts_.pop_front();
    }
    void draw(cv::Mat& img, const cv::Scalar& color = Palette::CYAN_SOFT) const {
        ...
        for (size_t i = 1; i < n; i++) {
            ...
            cv::line(img, cv::Point((int)pts_[i - 1].x, (int)pts_[i - 1].y),
                     cv::Point((int)pts_[i].x, (int)pts_[i].y), c, 1, cv::LINE_AA);
        }
    }
private:
    size_t max_len_;
...
int main() {
    ...
    cv::VideoWriter writer("output/kalman_ball_output.mp4", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), FPS, cv::Size(W, H));
    if (!writer.isOpened()) {
        writer.open("kalman_ball_output.mp4", cv::VideoWriter::fourcc('m', 'p', '4', 'v'), FPS, cv::Size(W, H));
    }
    ...
    skyvanta::hud::TrailRenderer measured_trail(60), filtered_trail(60);
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (bx < radius || bx > W - radius) { vx *= -1; bx = skyvanta::hud::clampd(bx, radius, W - radius); }
        ...
        measured_trail.draw(frame, skyvanta::hud::Palette::WHITE_DIM);
        filtered_trail.draw(frame, skyvanta::hud::Palette::CYAN);
        cv::circle(frame, cv::Point((int)mx, (int)my), 3, skyvanta::hud::Palette::AMBER, -1, cv::LINE_AA);
        skyvanta::hud::glow_circle(frame, filtered_pt, radius, skyvanta::hud::Palette::CYAN, 0.7);

        skyvanta::hud::rounded_rect(frame, {16, 16}, {350, 78}, skyvanta::hud::Palette::BG_PANEL, -1, 8, 0.5);
        skyvanta::hud::put_text(frame, "SKYVANTA C++ KALMAN DEMO", {28, 38}, 0.5, skyvanta::hud::Palette::WHITE, 1, true);
        skyvanta::hud::put_text(frame, "amber = noisy measurement, cyan = filtered estimate", {28, 60}, 0.38, skyvanta::hud::Palette::WHITE_DIM, 1, true);

        skyvanta::hud::frame_corners(frame);
        skyvanta::hud::scanline(frame, t_sec);
        if (writer.isOpened()) {
            writer.write(frame);
        }
    }
    ...
    std::cout << "[SkyVanta C++] Wrote kalman_ball_output.mp4 successfully.\n";
    return 0;
}
